...
public class AlgaeHexagonPositionCalculator {

  public static class AlgaeScoringPosition {
    public Translation2d position;
    public Rotation2d rotation;

    public AlgaeScoringPosition(Translation2d position, Rotation2d rotation) {
  ...
  public static ArrayList<AlgaeScoringPosition> calculateHexagonPositions(
      double centerblueX,
      double centerblueY,
      double centerredX,
      double centerredY,
    ...
    for (int side = 0; side < 6; side++) {
      ...
      if (side == 6) {
        ...
        if (redside) {
          ...
          positions.add(new AlgaeScoringPosition(new Translation2d(pos2X, pos2Y), rotation));
        } else {
  ...
  public static AlgaeScoringPosition findNearestPosition(
      Translation2d currentPosition,
      ArrayList<AlgaeScoringPosition> positions,
      double reefx,
      double reefy) {
    double minDistance = Double.MAX_VALUE;
    AlgaeScoringPosition nearestPosition = null;

    for (AlgaeScoringPosition position : positions) {
