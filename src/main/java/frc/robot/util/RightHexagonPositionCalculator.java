...
public class RightHexagonPositionCalculator {

  public static class RightScoringPosition {
    public Translation2d position;
    public Rotation2d rotation;

    public RightScoringPosition(Translation2d position, Rotation2d rotation) {
  ...
  public static ArrayList<RightScoringPosition> calculateHexagonPositions(
      double centerblueX,
      double centerblueY,
      double centerredX,
      double centerredY,
      ...
      double yOffset2,
      boolean redside) {
    ArrayList<RightScoringPosition> positions = new ArrayList<>();
    ...
    for (int side = 0; side < 7; side++) {
      ...
      if (side == 6) {
        if (redside) {
          ...
          // 4.0259 y
          positions.add(
              new RightScoringPosition(
                  new Translation2d(netredx, netredy1), Rotation2d.fromDegrees(0)));
          positions.add(
              new RightScoringPosition(
                  new Translation2d(netredx, netredy2), Rotation2d.fromDegrees(0)));
        } else {
          positions.add(
              new RightScoringPosition(
        ...
        if (redside) {
          ...
          positions.add(new RightScoringPosition(new Translation2d(pos1X, pos1Y), rotation));
  ...
  public static RightScoringPosition findNearestPosition(
      Translation2d currentPosition,
      ArrayList<RightScoringPosition> positions,
      double reefx,
      double reefy) {
    double minDistance = Double.MAX_VALUE;
    RightScoringPosition nearestPosition = null;

    for (RightScoringPosition position : positions) {
